...
var i, element, myfile, myurl, assetsDir;

for (i = 0; element = scriptElements[i]; i++) {

    myfile = element.src;

    if (myfile.indexOf('filemanager.js') >= 0) {
        var myurl = myfile.substring(0, myfile.indexOf('filemanager.js'));
    }
}
var string = myurl;
var arr = string.split('/');
//INFO: Will get value like assets/2e1daf3d
assetsDir = arr[arr.length - 4] + '/' + arr[arr.length - 3];
...
(function ($) {
    ...
    $.richFilemanagerPlugin = function (element, pluginOptions)
    {
        ...
        var configure = function () {
            return $.when(loadConfigFile('default'), loadConfigFile('user')).done(function (confd, confu) {
                ...
                if (config.api.connectorUrl) {
                    ...
                    var langConnector = assetsDir + '/connector/filemanager.php';
                ...
                //Start Own Script
                try{
                    if(fm.settings.config.api.connectorUrl !== undefined && trim(fm.settings.config.api.connectorUrl) != ''){
                        apiConnector = fm.settings.config.api.connectorUrl;
        ...
        var localize = function () {
            ...
            //Start
            try{               
                if(fm.settings.config.options.culture !== undefined && trim(fm.settings.config.options.culture) != '')
                    langCode = fm.settings.config.options.culture;
            } catch(e){
                //Do nothing
            }
            //End
        ...
        var includeAssets = function (callback) {
            ...
            try{
                if(fm.settings.config.options.theme !== undefined && trim(fm.settings.config.options.culture) != '')
                    theme = fm.settings.config.options.theme;
            } catch(e){                
                //Do nothing
            }
            //End
...
//INFO START THIS
            ...
            //INFO: Own script
            try{
                if(fm.settings.config.customScrollbar.enabled !== undefined && trim(fm.settings.config.customScrollbar.enabled) != ''){
                    if(fm.settings.config.customScrollbar.enabled == true){
                        primary.push('/scripts/custom-scrollbar-plugin/jquery.mCustomScrollbar.min.css');
                        primary.push('/scripts/custom-scrollbar-plugin/jquery.mCustomScrollbar.concat.min.js');
                    }else{
                        index = primary.indexOf('/scripts/custom-scrollbar-plugin/jquery.mCustomScrollbar.min.css');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/custom-scrollbar-plugin/jquery.mCustomScrollbar.concat.min.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                    }
            ...
            //INFO: Own script
            try{
                if(fm.settings.config.viewer.editable.enabled !== undefined && trim(fm.settings.config.viewer.editable.enabled) != ''){
                    try{
                        var editorTheme = fm.settings.config.viewer.editable.theme;
                        if (editorTheme !== undefined && trim(editorTheme) != '' && editorTheme && editorTheme !== 'default') {
                            secondary.push('/scripts/CodeMirror/theme/' + editorTheme + '.css');
                        }
                    } catch(e){                
                        //Do nothing
                    ...
                    if(fm.settings.config.viewer.editable.enabled == true){
                        secondary.push('/scripts/CodeMirror/lib/codemirror.css');
                        secondary.push('/scripts/CodeMirror/lib/codemirror.js');
                        secondary.push('/scripts/CodeMirror/addon/selection/active-line.js');
                        secondary.push('/scripts/CodeMirror/addon/display/fullscreen.css');
                        ...
                        index = primary.indexOf('/scripts/CodeMirror/lib/codemirror.css');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/CodeMirror/lib/codemirror.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/CodeMirror/addon/selection/active-line.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/CodeMirror/addon/display/fullscreen.css');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/CodeMirror/addon/display/fullscreen.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                    }
            ...
            //INFO: Own script
            try{
                if(fm.settings.config.viewer.markdownRenderer.enabled !== undefined && trim(fm.settings.config.viewer.markdownRenderer.enabled) != ''){                    
                    if(fm.settings.config.viewer.markdownRenderer.enabled == true){
                        secondary.push('/styles/fm-markdown.css');
                        secondary.push('/scripts/markdown-it/markdown-it.min.js');
                        secondary.push('/scripts/markdown-it/default.min.css');
                        secondary.push('/scripts/markdown-it/highlight.min.js');
                        ...
                        index = primary.indexOf('/styles/fm-markdown.css');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/markdown-it/markdown-it.min.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/markdown-it/default.min.css');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/markdown-it/highlight.min.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/markdown-it/markdown-it-footnote.min.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/markdown-it/markdown-it-replace-link.min.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                    }
            ...
            //INFO: Own script
            try{
                if(fm.settings.config.options.browseOnly !== undefined && trim(fm.settings.config.options.browseOnly) != ''){                    
                    if(fm.settings.config.options.browseOnly == false){
                        // Loading jquery file upload library
                        secondary.push('/scripts/jQuery-File-Upload/js/vendor/jquery.ui.widget.js');
                        secondary.push('/scripts/jQuery-File-Upload/js/canvas-to-blob.min.js');
                        secondary.push('/scripts/jQuery-File-Upload/js/load-image.all.min.js');
                        ...
                        try{
                            if(fm.settings.config.upload.multiple !== undefined && trim(fm.settings.config.upload.multiple) != ''){
                                if(fm.settings.config.upload.multiple == true){
                                    secondary.push('/scripts/jQuery-File-Upload/css/dropzone.css');
                                }else{
                                    index = primary.indexOf('/scripts/jQuery-File-Upload/css/dropzone.css');
                                    if (index > -1) {
                                        primary.splice(index, 1);
                                    }
                                }
                        ...
                        index = primary.indexOf('/scripts/jQuery-File-Upload/js/vendor/jquery.ui.widget.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/jQuery-File-Upload/js/canvas-to-blob.min.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/jQuery-File-Upload/js/load-image.all.min.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/jQuery-File-Upload/js/jquery.iframe-transport.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/jQuery-File-Upload/js/jquery.fileupload.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/jQuery-File-Upload/js/jquery.fileupload-process.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        
                        index = primary.indexOf('/scripts/jQuery-File-Upload/js/jquery.fileupload-image.js');
                        if (index > -1) {
                            primary.splice(index, 1);
                        }
                        index = primary.indexOf('/scripts/jQuery-File-Upload/css/dropzone.css');
        ...
        //TODO: will make accessible conf
        ...
        function has_capability(resourceObject, cap) {
            ...
            if (cap === 'select' && resourceObject.type === 'folder')
                return false;
            if (cap === 'replace' && resourceObject.type === 'folder')
                return false;
            if (cap === 'extract') {
        ...
        var isAuthorizedFile = function (filename) {
            ...
            if (config.upload.policy == 'DISALLOW_ALL') {
                if ($.inArray(ext, config.upload.restrictions) !== -1)
                    return true;
            }
